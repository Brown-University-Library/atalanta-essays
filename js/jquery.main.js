...
function refreshCollections() {
	...
	jQuery('.all-collections .collection-wrapper').each(function() {
		...
		collection.find('[data-emblem-id]').each(function() {
			...
			emblemsArr.push(curEmblem.data('emblemId'));
		});
...
function initAddEmblem(name, arr) {
	...
	for (var i = 0; i < curCollections.length; i++) {
...
// init json test
function initAppendEmblem(id, index, counter, max, size) {
					...
					case 'add':
						...
						if (curCollections && curCollections.length) {
							...
							collectionList.on('click', 'a', function(e) {
								...
								for (var j = 0; j < newEmblems.length; j++) {
									var id = ('0' + newEmblems[j]).slice(-2);
									initAppendEmblem(id, curLink.closest('li').index(), j, newEmblems.length, cropSize);
								}
					...
					case 'add-new':
						...
						addLink.on('click', function(e) {
							...
							items.filter('.' + activeClass).each(function() {
								var item = jQuery(this);
								arr.push(item.find('.img-holder').data('emblemId'));
							});
							...
							if (arr.length) {
								...
								for (var j = 0; j < arr.length; j++) {
									var id = ('0' + arr[j]).slice(-2);
									initAppendEmblem(id, instance.$trigger.closest('.collection-wrapper').index(), j, arr.length, size);
								}
							}
...
;(function($) {
	...
	AnimCircles.prototype = {
		...
		attachEvents: function() {
			...
			self.resizeHandler();
			$(window).on('resize orientationchange', self.resizeHandler);
		...
		onResize: function(self) {
			self.holder.css({
				'height': window.innerHeight * (self.steps.length ? self.steps.length : 1)
			});
		...
		onScroll: function(self) {
			...
			if (winTop >= holderTop && winTop + window.innerHeight < holderTop + self.holder.outerHeight()) {
				...
				newStep = Math.ceil((winTop - holderTop + window.innerHeight / 2) / window.innerHeight);
...
;(function($) {
	...
	AnimSection.prototype = {
		...
		attachEvents: function() {
			...
			self.children.wrap('<div class="anim-section-wrapper"></div>');
		...
		onResize: function(self) {
			...
			self.winHeight = window.innerHeight;
			self.holder.css({
				'height': self.winHeight * (self.steps.count * self.koef)
			});
		...
		onScroll: function(self) {
			...
			if (winTop >= self.holderTop && winTop + self.winHeight < self.holderTop + self.winHeight * (self.steps.count * self.koef)) {
				self.holder.removeClass(self.options.scrollClass);
				self.holder.addClass(self.options.fixedClass);
				self.curIndex = parseInt((winTop - self.holderTop) / (self.winHeight * self.koef));
			} else {
				if (winTop + self.winHeight >= self.holderTop + self.winHeight * (self.steps.count * self.koef)) {
					self.holder.addClass(self.options.scrollClass);
		...
		destroy: function() {
			...
			setTimeout(function() {
				...
				self.children.unwrap('<div class="anim-section-wrapper"></div>');
